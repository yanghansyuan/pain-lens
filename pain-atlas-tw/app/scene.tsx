...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  const pulseUniforms:{value:number}[]=[];
  ...
  const locatorPositions=new Float32Array(atlas.parts.length*3),locatorGeometry=new T.BufferGeometry();locatorGeometry.setAttribute('position',new T.BufferAttribute(locatorPositions,3));
  const locatorMaterial=new T.PointsMaterial({color:0xffd447,size:18,sizeAttenuation:false,transparent:true,opacity:.9,depthTest:false});
  locatorMaterial.onBeforeCompile=shader=>{shader.fragmentShader=shader.fragmentShader.replace('#include <clipping_planes_fragment>','#include <clipping_planes_fragment>\nif (distance(gl_PointCoord, vec2(0.5)) > 0.5) discard;');};
  const locator=new T.Points(locatorGeometry,locatorMaterial);locator.frustumCulled=false;locator.renderOrder=20;locator.visible=false;scene.add(locator);
  const hover=document.createElement('div');hover.className='part-hover';hover.setAttribute('role','tooltip');hover.hidden=true;el.appendChild(hover);
  const locatorLabel=document.createElement('div');locatorLabel.className='part-locator';locatorLabel.textContent='這裡';locatorLabel.hidden=true;el.appendChild(locatorLabel);
  ...
  const materialFor=(system:string)=>{
   ...
   m.onBeforeCompile=shader=>{
    const selectedPulse={value:0};pulseUniforms.push(selectedPulse);
    shader.uniforms.partState={value:partTexture};shader.uniforms.selectionState={value:selectionTexture};shader.uniforms.stateWidth={value:width};shader.uniforms.selectedPulse=selectedPulse;
  ...
  const clock=new T.Clock();let lastExtent=-1,lastHighlight=0,highlightUntil=0;
  const animate=()=>{
   if(disposed)return;frame=requestAnimationFrame(animate);const dt=Math.min(clock.getDelta(),.05),s=latest.current,now=clock.elapsedTime;
   if(s.highlight!==lastHighlight){highlightUntil=now+3.2;lastHighlight=s.highlight;dirty=true;}
   const locating=Math.max(0,highlightUntil-now),pulse=locating>0?(.55+.45*Math.sin(now*10))*Math.min(1,locating):0;
   if(locating>0)dirty=true;
   pulseUniforms.forEach(u=>u.value=pulse);
   ...
   if(changed||moving||lastExtent<0){
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     const selected=selection.has(p.id)&&!hidden.has(p.id);data.set([dx,dy,dz,(!hidden.has(p.id)&&(s.isolate?selected:visible.has(p.system)||selected))?1:0],i*4);selectedData[i*4]=selected?255:0;
     const position=data[i*4+3]>.5?[c.x+dx,c.y+dy,c.z+dz]:[10000,10000,10000];
     markerPositions.set(position,i*3);locatorPositions.set(selected&&locating>0?position:[10000,10000,10000],i*3);const mesh=pickers[i];if(mesh){mesh.position.set(dx,dy,dz);mesh.updateMatrix();mesh.updateMatrixWorld(true);}
    });partTexture.needsUpdate=true;selectionTexture.needsUpdate=true;markerGeometry.attributes.position.needsUpdate=true;lastState=s;lastExtent=amount;dirty=true;
   }
   locator.visible=locating>0&&s.selected.length>0;locatorMaterial.opacity=.3+pulse*.7;locatorMaterial.size=18+pulse*18;locatorGeometry.attributes.position.needsUpdate=true;
   locatorLabel.hidden=true;
   if(locating>0&&s.selected.length>0){const selectedIndexes=atlas.parts.map((p,i)=>s.selected.includes(p.id)?i:-1).filter(i=>i>=0);if(selectedIndexes.length){const center=new T.Vector3();selectedIndexes.forEach(i=>center.add(centers[i]).add(new T.Vector3(data[i*4],data[i*4+1],data[i*4+2])));center.multiplyScalar(1/selectedIndexes.length);projected.copy(center).project(camera);if(projected.z>=-1&&projected.z<=1){locatorLabel.hidden=false;locatorLabel.style.left=`${(projected.x+1)*el.clientWidth/2}px`;locatorLabel.style.top=`${(1-projected.y)*el.clientHeight/2}px`;}}}
